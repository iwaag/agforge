...
class AgentConfigError(Exception):
    """Stable-code configuration or availability failure."""
...
def _validate_overlay(overlay: dict[str, Any]) -> None:
    ...
    local_extras = set(local) - {"harness", "provider", "secrets"}
    if local_extras:
        raise AgentConfigError(
            "E_OVERLAY_SCOPE", f"overlay local keys not allowed: {sorted(local_extras)}"
        )
    for harness, facts in _table(local, "harness").items():
        ...
        if bool(facts.get("command")) == bool(facts.get("command_glob")):
            raise AgentConfigError(
                "E_OVERLAY_SCOPE",
                f"local.harness.{harness} must set exactly one of command or command_glob",
            )
    ...
    for key, value in _table(local, "secrets").items():
        if not key.endswith(("_file", "_env")):
            raise AgentConfigError("E_SECRET_VALUE", f"secret {key!r} must be a reference")
        if not isinstance(value, str):
            raise AgentConfigError("E_SECRET_VALUE", f"secret reference {key!r} must be a string")
...
def _validate_committed(config: dict[str, Any]) -> None:
    ...
    for role, settings in roles.items():
        ...
        if settings.get("profile") not in profiles:
            raise AgentConfigError(
                "E_UNKNOWN_PROFILE", f"role {role!r}: unknown profile {settings.get('profile')!r}"
            )
...
def _resolve_command(harness: str, overlay: dict[str, Any], *, check_available: bool) -> str:
    ...
    command = os.path.expanduser(command or {"opencode": "opencode", "claude_code": "claude"}.get(harness, ""))
...
def resolve_role(
    ...
    *,
    ...
    provider_facts = _table(_table(_table(overlay, "local"), "provider"), provider)
    provider_base_url = provider_facts.get("base_url")
    if check_available and harness == "opencode" and provider == "ollama" and not provider_base_url:
        raise AgentConfigError(
            "E_UNAVAILABLE", "local.provider.ollama.base_url is required by agforge OpenCode"
        )
    return ResolvedAgent(
        ...
        provider=provider,
        model=model,
        model_options=dict(_table(config, "models")[model]),
        command=_resolve_command(harness, overlay, check_available=check_available),
