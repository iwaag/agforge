...
def run_agent(charter: str, timeout: float) -> tuple[str, dict]:
    ...
    infra failures, not ENT problem reports.
    """
    if timeout <= 0:
    ...
    if proc.returncode != 0:
        tail = (output or proc.stderr or "agent produced no output").strip()
        raise AgentRunError(
            f"agent exited {proc.returncode}: {tail[-OUTPUT_TAIL_CHARS:]}"
        )
    if not output.strip():
...
def run_request(
    ...
    try:
        output, meta = run_agent(charter, timeout=budget_seconds)
